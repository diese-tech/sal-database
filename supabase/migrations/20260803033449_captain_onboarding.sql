...
BEGIN
  ...
  IF FOUND THEN
    IF v_season_org.division_id <> v_division_id
      OR v_season_org.status <> 'active' THEN
      RAISE EXCEPTION USING
        ERRCODE = '23514',
        MESSAGE = format(
          'Organization %s already has a conflicting assignment in season %s',
          v_org_id,
          v_season_id
        );
    END IF;
  ELSE
    ...
    INSERT INTO public.audit_logs (
      ...
      'Created the explicit organization mapping required for captain onboarding.'
    );
...
COMMENT ON FUNCTION public.onboard_season_captain(text, text, text, text, text, text)
  IS 'Idempotently creates one explicit season captain mapping after Discord identity linking and appends immutable audits.';
