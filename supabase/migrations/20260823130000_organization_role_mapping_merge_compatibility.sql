-- Keep organization identity merges compatible with Discord authority and
-- season-team projection mappings. The canonical target mapping wins when both
-- identities already have one; otherwise the source mapping is re-keyed.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'seasonTeamRoleMappings', (
      SELECT count(*) FROM public.season_organization_role_mappings WHERE org_id = v_source_id
    ),
    'organizationAuthorityRoleMappings', (
      SELECT count(*) FROM public.organization_role_mappings WHERE org_id = v_source_id
    ),
...
BEGIN
  ...
  -- Preserve the canonical target mapping where both identities already have
  -- one. Otherwise re-key the source mapping before its parent row is removed.
  DELETE FROM public.organization_role_mappings AS source_mapping
  WHERE source_mapping.org_id = v_source_id
    AND EXISTS (
      SELECT 1 FROM public.organization_role_mappings AS target_mapping
      WHERE target_mapping.org_id = v_target_id
    );
  UPDATE public.organization_role_mappings
  SET org_id = v_target_id, updated_by_discord_id = v_actor_id, updated_at = now()
  WHERE org_id = v_source_id;

  DELETE FROM public.season_organization_role_mappings AS source_mapping
  WHERE source_mapping.org_id = v_source_id
    AND EXISTS (
      SELECT 1
      FROM public.season_organization_role_mappings AS target_mapping
      WHERE target_mapping.season_id = source_mapping.season_id
        AND target_mapping.division_id = source_mapping.division_id
        AND target_mapping.org_id = v_target_id
    );
  UPDATE public.season_organization_role_mappings
  SET org_id = v_target_id, updated_by_discord_id = v_actor_id, updated_at = now()
  WHERE org_id = v_source_id;
