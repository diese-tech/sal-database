-- Consolidate duplicate player identities created when a Discord registration
-- was approved instead of being linked to an imported league identity.
--
-- The target owns profile and competitive metadata. The source contributes
-- only missing Discord linkage/avatar data, while every typed live reference
-- is moved transactionally. Immutable audit, outbox, and correction evidence
-- is intentionally retained with the historical source ID.

CREATE SCHEMA IF NOT EXISTS private;
REVOKE ALL ON SCHEMA private FROM PUBLIC, anon, authenticated;

-- Preserve the reviewed implementation behind a private boundary, then place
-- a username-linking invariant in front of it. This keeps all existing
-- validation/audit behavior while preventing approval from creating a player
-- when exactly one active, unclaimed imported identity already owns the
-- normalized Discord username.
ALTER FUNCTION public.resolve_registration_review(text, text, text, text)
  RENAME TO resolve_registration_review_unhardened;
ALTER FUNCTION public.resolve_registration_review_unhardened(text, text, text, text)
  SET SCHEMA private;
REVOKE ALL ON FUNCTION private.resolve_registration_review_unhardened(text, text, text, text)
...
DECLARE
  ...
  v_candidate public.players%ROWTYPE;
  v_candidate_id text;
...
BEGIN
  ...
  IF FOUND
    AND lower(btrim(COALESCE(p_decision, ''))) = 'approve'
    AND v_registration.status = 'pending'
    AND v_registration.player_id IS NULL
    AND EXISTS (
    ...
    v_username := lower(btrim(COALESCE(v_registration.discord_username, '')));

    IF v_username <> '' THEN
      PERFORM pg_advisory_xact_lock(
        hashtextextended('registration-player-username:' || v_username, 0)
      );

      -- Lock every candidate before counting so concurrent reviews cannot both
      -- decide that the same imported identity is available.
      PERFORM id
      FROM public.players
      WHERE lower(btrim(discord_username)) = v_username
      ...
      IF v_match_count > 1 THEN
        RAISE EXCEPTION USING
          ERRCODE = '23514',
          MESSAGE = 'Registration approval blocked [REGISTRATION_USERNAME_AMBIGUOUS]: multiple active players match the normalized Discord username.';
      ELSIF v_match_count = 1 THEN
        SELECT * INTO v_candidate
        FROM public.players
        WHERE id = v_candidate_id;

        IF v_candidate.discord_id IS NULL AND NOT v_candidate.profile_claimed THEN
          UPDATE public.registrations
          SET player_id = v_candidate.id
          WHERE id = v_registration.id;
        ELSIF v_candidate.discord_id = v_registration.discord_id THEN
          UPDATE public.registrations
          SET player_id = v_candidate.id
          WHERE id = v_registration.id;
        ELSE
          RAISE EXCEPTION USING
            ERRCODE = '23514',
            MESSAGE = 'Registration approval blocked [REGISTRATION_USERNAME_CONFLICT]: the normalized Discord username belongs to a claimed player.';
        END IF;
      END IF;
    END IF;
  END IF;

  v_result := private.resolve_registration_review_unhardened(
    p_registration_id,
    p_actor_discord_id,
    p_decision,
    p_reviewer_note
  ...
  IF lower(btrim(COALESCE(p_decision, ''))) = 'approve'
    AND COALESCE((v_result ->> 'applied')::boolean, false)
    AND v_result ->> 'playerId' IS NOT NULL
    AND v_registration.avatar_url IS NOT NULL THEN
    UPDATE public.players
    SET avatar_url = COALESCE(avatar_url, v_registration.avatar_url)
    WHERE id = (v_result ->> 'playerId');
  END IF;
...
COMMENT ON FUNCTION public.resolve_registration_review(text, text, text, text) IS
  'Atomically resolves a registration review and reuses one normalized active unclaimed player identity before any new player can be created.';
...
ALTER FUNCTION public.preview_player_merge(text, text) OWNER TO postgres;
ALTER FUNCTION public.merge_player(text, text, text) OWNER TO postgres;

REVOKE ALL ON FUNCTION public.preview_player_merge(text, text)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.merge_player(text, text, text)
  FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.preview_player_merge(text, text)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.merge_player(text, text, text)
  TO service_role;

COMMENT ON FUNCTION public.preview_player_merge(text, text) IS
  'Returns safe identities, affected-row counts, and fail-closed blockers for consolidating a duplicate player.';
COMMENT ON FUNCTION public.merge_player(text, text, text) IS
  'Atomically coalesces compatible player history, transfers every typed live reference, preserves immutable evidence, audits the actor, and deletes the duplicate.';
